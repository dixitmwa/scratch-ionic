...
export default function (vm: any, useCatBlocks: any) {
    ...
    const jsonForMenuBlock = function (name: string, menuOptionsFn: { (): string[][]; (): any; (): any; (): any[][]; (): any[][]; (): any[][]; (): any[][]; (): any[][]; (): any[][]; (): any[][]; (): any; }, colors: { secondary: any; tertiary: any; quaternary: any; }, start: string | any[]) {
        return {
            message0: '%1',
            args0: [
                {
    ...
    const soundsMenu = function () {
        ...
        if (vm.editingTarget && vm.editingTarget.sprite.sounds.length > 0) {
            menu = vm.editingTarget.sprite.sounds.map((sound: { name: any; }) => [sound.name, sound.name]);
        }
        menu.push([
